...
function SERVICE:Match( url )
	return string.match(url.host, "cokencorn.com") and
...
function SERVICE:GetVideoInfo( data, onSuccess, onFailure )
	...
	local onReceive = function( body, length, headers, code )
		...
		if response.status and response.status == 404 then
			return onFailure("CokeNCorn.Com : " .. tostring(response.message))
		end
...
SERVICE.Name 			= "CokeNCorn.Com Stream"
SERVICE.IsTimed 		= false
SERVICE.TheaterType 	= THEATER_PRIVATE

function SERVICE:Match( url )
	return string.match( url.host, "cokencorn.com" )
end
...
function SERVICE:GetVideoInfo( data, onSuccess, onFailure )
	...
	local onReceive = function( body, length, headers, code )
		...
		elseif response.game then
			info.title = "CokeNCorn.Com : " .. response.game
		else
			info.title = "CokeNCorn.Com Stream"
		end
...
theater.RegisterService( 'cokencornstream', SERVICE )
